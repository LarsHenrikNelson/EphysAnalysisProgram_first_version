...
class FinalMiniAnalysis:
    ...
    def analyze_average_mini(self):
        ...
        popt, pcov = curve_fit(
            self.decay_exp, self.decay_x, self.decay_y, p0=init_param, bounds=bounds
        )
        fit_amp, self.fit_tau_x = popt
        self.fit_decay_y = self.decay_exp(self.decay_x, fit_amp, self.fit_tau_x)
...
class FinalCurrentClampAnalysis:
    ...
    def final_data(self):
        ...
        if "Peak_AHP (mV)" in self.final_df.columns.levels[0].tolist():
        ...
        if "Peak_AHP (ms)" in self.final_df.columns.levels[0].tolist():
