...
class currentClampWidget(QWidget):
    ...
    def __init__(self):
        ...
        self.analysis_buttons.addRow(self.acquisition_number_label, 
        ...
        self.analysis_buttons.addRow(self.baseline_mean_label,
                                     self.baseline_mean_edit)
        
        self.delta_v_label = QLabel('Delta V')
        self.delta_v_edit = QLineEdit()
        self.analysis_buttons.addRow(self.delta_v_label,
                                     self.delta_v_edit)
        
        self.spike_threshold_label = QLabel('Spike threshold')
        self.spike_threshold_edit = QLineEdit()
        self.analysis_buttons.addRow(self.spike_threshold_label,
                                     self.spike_threshold_edit)
        
        self.spike_rate_label = QLabel('Spike rate')
        self.spike_rate_edit = QLineEdit()
        self.analysis_buttons.addRow(self.spike_rate_label,
                                     self.spike_rate_edit)
        ...
        self.reset_recent_reject_button.clicked.connect(
            self.reset_recent_reject_acq)
        self.analysis_buttons.addRow(self.reset_recent_reject_button)
        
        self.reset_rejected_acqs_button = QPushButton(
            'Reset rejected acquistions')
        self.reset_rejected_acqs_button.clicked.connect(
            self.reset_rejected_acqs)
        ...
        self.pulse_start_label = QLabel('Pulse start')
        ...
        self.pulse_end_label = QLabel('Pulse end')
        ...
        self.ramp_start_label = QLabel('Ramp start')
        ...
        self.ramp_end_label = QLabel('Ramp end')
        ...
        self.min_spike_threshold_label = QLabel('Min spike threshold')
        ...
        self.input_layout.addRow(self.min_spike_threshold_label, 
                                 self.min_spike_threshold_edit)
    ...
    def spinbox(self, h):
        ...
        if (int(self.acquisition_number.text()) in self.analysis_list):
            ...
            self.baseline_mean_edit.setText(str(
                self.round_sig(acq_object.baseline_mean, sig=4)))
            self.delta_v_edit.setText(str(self.round_sig(acq_object.delta_v, 
                                                          sig=4)))
            self.spike_threshold_edit.setText(str(
                self.round_sig(acq_object.spike_threshold, sig=4)))
            self.spike_rate_edit.setText(str(
                self.round_sig(acq_object.hertz_exact, sig=4)))
            if acq_object.ramp == '0':
                self.plot_widget.plot(x=acq_object.x_array, 
                                      y=acq_object.array)
                self.plot_widget.plot(x=acq_object.plot_x, 
                                      y=acq_object.plot_y, 
                                      pen='r')
                if acq_object.peaks[0] is not np.nan:
                    self.plot_widget.plot(
                        ...
                        y=acq_object.array[acq_object.peaks], pen=None,
                        symbol = 'o', symbolBrush='y')
                    self.plot_widget.plot(
                        x=[acq_object.rheo_x/acq_object.s_r_c],
                        y=[acq_object.spike_threshold], pen=None, symbol='o',
                        symbolBrush='b')
                ...
                if len(acq_object.peaks) > 0:
                    self.plot_widget.plot(
                        x = acq_object.peaks/acq_object.s_r_c, 
                        y = acq_object.array[acq_object.peaks], pen=None,
                        symbol = 'o', symbolBrush='y')
                    self.plot_widget.plot(
                        x=[acq_object.rheo_x/acq_object.s_r_c],
    ...
    def plot_iv_curve(self):
        ...
        if len(self.final_obj.plot_epochs) == 1:
            self.iv_curve_plot.plot(iv_df['iv_plot_x'],
                iv_df[self.final_obj.plot_epochs[0]])
            self.iv_curve_plot.plot(deltav_df['deltav_x'],
                deltav_df[self.final_obj.plot_epochs[0]],
                pen=None, symbol = 'o', symbolBrush='y')
        else:
            ...
            for i in self.final_obj.plot_epochs:
                if iv_df[i].isna().all():
                    ...
                    self.iv_curve_plot.plot(iv_df['iv_plot_x'], iv_df[i],
                        pen=pencil)
                    self.iv_curve_plot.plot(deltav_df['deltav_x'],
                        deltav_df[i], pen=None, symbol = 'o', symbolPen=pencil,
                        symbolBrush=brush, 
                        name=f'Epoch {i}')
    ...
    def plot_spike_frequency(self, df):
        ...
        if df1.empty == True or 'Hertz' not in df1.columns.levels[0].tolist():
            ...
            if len(self.hertz_y) == 1:
                self.spike_curve_plot.plot(self.hertz_x, self.hertz_y[0],
                    symbol = 'o', name=f'Epoch {plot_epochs[0]}')
            else:
                ...
                for i in range(len(self.hertz_y)):
                    if np.isnan(self.hertz_y).all():
                        ...
                        pencil = pg.mkPen(color=pg.intColor(i))
                        brush = pg.mkBrush(color=pg.intColor(i))
                        self.spike_curve_plot.plot(self.hertz_x,
                            self.hertz_y[i], symbol='o', pen=pencil,
    ...
    def plot_pulse_ap(self, df):
       if len(df.columns) > 1:
           for i in df.columns:
               ...
               array = df[i].to_numpy()
               pencil = pg.mkPen(color=pg.intColor(i))
               self.pulse_ap_plot.plot(np.arange(len(array))/10, array, 
                        pen=pencil, name=f'Epoch {i}')
       else:
           i = df.columns[0]
           array = df[i]
           self.pulse_ap_plot.plot(np.arange(len(array))/10, array, 
    ...
    def plot_ramp_ap(self, df):
       if len(df.columns) > 1:
           for i in df.columns:
               ...
               array = df[i].to_numpy()
               pencil = pg.mkPen(color=pg.intColor(i))
               self.ramp_ap_plot.plot(np.arange(len(array))/10, array, 
                        pen=pencil, name=f'Epoch {i}')
       else:
           i = df.columns[0]
           array = df[i]
    ...
    def open_files(self):
        ...
        if not file_list:
            ...
            self.acquisition_number.setMaximum(
                int(list(self.acq_dict.keys())[-1]))
            self.acquisition_number.setMinimum(
                int(list(self.acq_dict.keys())[0])) 
            self.analysis_list = np.arange(int(list(self.acq_dict.keys())[0]),
                int(list(self.acq_dict.keys())[-1])+1).tolist()
            self.acquisition_number.setValue(
                int(list(self.acq_dict.keys())[0]))
            self.raw_list = [self.acq_dict[i].current_clamp_dict for i in \
                          self.acq_dict.keys()]
