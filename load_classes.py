...
class LoadCurrentClampData:
    def __init__(self, excel_file):
        save_values = pd.read_excel(excel_file, sheet_name=['Raw data',
                        'Pulse APs', 'Ramp APs'])
        self.raw_df = save_values['Raw data']
        self.final_df = pd.read_excel(excel_file, sheet_name='Final data', 
                     header=[0,1]).drop(labels=0)
        self.final_df.rename(columns={'Unnamed: 2_level_1': '',
                                 'Unnamed: 1_level_1': ''}, level=1,
                             inplace=True)
        self.final_df.rename(columns={'Unnamed: 0_level_0': ''}, level=0,
                             inplace=True)
