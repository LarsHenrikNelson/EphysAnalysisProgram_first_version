...
class AcqInspectionWidget(QWidget):
    def __init__(self):
        ...
        self.plot_widget = pg.PlotWidget(labels={'left': 'Amplitude (pA)', 'bottom': 'Samples'})
    ...
    def spinbox(self, number):
        ...
        self.plot_widget.plot(x=np.arange(len(self.acq_dict[str(number)][2])),
            y = self.acq_dict[str(number)][2])
